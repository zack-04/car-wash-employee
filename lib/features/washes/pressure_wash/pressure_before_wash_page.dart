...
import 'package:car_wash_employee/features/washes/pressure_wash/pressure_wash_timer_page.dart';
...
class PressureBeforeWashPage extends ConsumerStatefulWidget {
  const PressureBeforeWashPage({
    super.key,
  ...
  @override
  ConsumerState<PressureBeforeWashPage> createState() =>
      _PressureBeforeWashPageState();
}

class _PressureBeforeWashPageState
    extends ConsumerState<PressureBeforeWashPage> {
  int _currentIndex = 0;
  File? _capturedImage;
  ...
  void handleResponse(WashResponse response) {
    Pattern pattern1 = widget.assignedCar.washName == pressureWashWithInterior
        ? response.data[1]
        : response.data[0];
    ...
    setState(() {
      ...
      midViews = allViews.skip(1).take(2).toList();
      afterViews = allViews.skip(3).take(8).toList();
  ...
  Future<void> distance() async {
    ...
    print('Previous - ${previousId.carId}');
  ...
  void _nextView() async {
    ...
    if (_currentIndex < beforeViews.length - 1) {
      ...
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(
          builder: (context) => PressureWashTimerPage(
